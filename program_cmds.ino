...
void cmd_cv(void) {
  ...
  if (rx_ptr.buf[0] == OPC_WCVOA)
  {
    ops_address.addr_hi.byte = rx_ptr.buf[1];
    ops_address.addr_lo = rx_ptr.buf[2];
    cvh = rx_ptr.buf[3];
    cvl = rx_ptr.buf[4];
    cv_data = rx_ptr.buf[6];
    mode = rx_ptr.buf[5];
  }
  else
  {
    ...
    mode = ( rx_ptr.buf[0] == OPC_WCVO ? 1 : 2 );
  }

  // Remove offset from CV address
  cv = (((unsigned int)cvh) << 8 | cvl) - 1;

  if ((rx_ptr.buf[0] == OPC_WCVO) || (rx_ptr.buf[0] == OPC_WCVB) || (rx_ptr.buf[0] == OPC_WCVOA)) {
    ...
    cv_data = rx_ptr.buf[5];                                                // Data value for WCVS
  ...
  switch (prog_state) {
    ...
    case CV_CHECK_OVLD:
      ...
      if (dcc_flags.dcc_overload == 1) {
        ...
        iccq = ave;           // Reset background current level for ACK comparison
